...
\begin{drv}
Writing the RSPT wavefunction equation as $\Y=\sum_{k=0}^{\infty}(R_0V_\mr{c} - R_0 E_\mr{c})^k\F$, note that, if $R_0$ and $V_\mr{c}$ were to commute, we could apply the binomial theorem to write the right-hand side as $\sum_{p=0}^k{k\choose p}(-)^p(R_0V_\mr{c})^{k-p}(R_0E_\mr{c})^p$.
Since they don't commute, we have to settle for the following generalization of the binomial expansion
\begin{align*}
  (R_0E_\mr{c} - R_0V_\mr{c})^k
=
  \sum_{p=0}^k
  (-)^p
  \{R_0E_\mr{c}\}^p\mr{insert}\{R_0V_\mr{c}\}^{k-p}
\end{align*}
...
\\=&\
  ...
  (-)^m
  \{R_0E_\mr{c}\ord{r_1},\ld,R_0E_\mr{c}\ord{r_m}\}
  \mr{insert}
  \{R_0V_\mr{c}\}^{n_2}
  \F
=
...
$
  (-)^k
  \{R_0E_\mr{c}\ord{n_1},\ld,R_0E_\mr{c}\ord{n_k}\}
  \mr{insert}
  \{R_0V_\mr{c}\}^{n_{k+1}}
  \F
$
...
+
  ...
  \F
\end{align*}
which can be evaluated as the sum of a \emph{principal term}, $(R_0V_\mr{c})^n\F$, plus all possible $m$-tuple substitutions of adjacent factors $(R_0V_\mr{c})^{r_k}$ in the principal term by $R_0E_\mr{c}\ord{r_k}$ times a sign factor $(-)^m$.
}
\end{lem}
...
\thmstatement{
The $n\eth$-order contribution to the wavefunction consists of a principal term $(R_0V_\mr{c})^n\F=R_0V_\mr{c}\cd R_0V_\mr{c}\F$ plus the sum over all possible ways of inserting one or more pairs of brackets $\ip{\cd}\equiv\ip{\F|\cd|\F}$ into the principal term, $R_0V_\mr{c}\cd R_0\ip{V_\mr{c}\cd R_0V_\mr{c}}\cd R_0V_\mr{c}\F$, allowing nested brackets.
Each of these terms gets a phase factor $(-)^k$ where $k$ is the total number of brackets.
}
\thmproof{
  This obviously holds for $\Y\ord{1}$ since $\Y\ord{1}=R_0V_\mr{c}\F$ and there are no possible bracketings.
  Assume it holds up to $n-1$ and consider $n$.
  By the substitution lemma, $\Y\ord{n}$ equals a principal term $R_0V_\mr{c}\cd R_0V_\mr{c}\F$ plus all unique substitutions of factors $(R_0V_\mr{c})^{r_1},\ld,(R_0V_\mr{c})^{r_m}$ in the principal term with energy factors $R_0E_\mr{c}\ord{r_1},\ld,R_0E_\mr{c}\ord{r_m}$, weighted by a sign $(-)^m$.
  But, by our inductive assumption, the substituted energies $E_\mr{c}\ord{r_k}=\ip{\F|V_\mr{c}|\Y\ord{r_k}}$ are sums of a principal term $\ip{V_\mr{c}R_0V_\mr{c}\cd R_0V_\mr{c}}$ plus all possible bracketings, with the appropriate sign factor, which shows that $\Y\ord{n}$ is the sum over all nested bracketings and completes the proof.
}
\end{thm}
...
\thmproof{
  If $\Y\ord{n}$ is linked then every unlinked contribution to $V_\mr{c}\Y\ord{n}$ has an open part and vanishes in the energy expression $E\ord{n+1}=\ip{\F|V_\mr{c}|\Y\ord{n}}=\ip{\F|V_\mr{c}|\Y\ord{n}}_{\mr{L}}$.
  ...
  By the energy substitution lemma, we have a sum over all possible $m$-tuple substitutions of adjacent factors $(R_0V_\mr{c})^{r}$ in the principale term by $R_0E_\mr{c}^{(r)}$ times a sign factor $({-})^m$ for the number of substitutions.
  By our inductive assumption, each substituted energy $E_\mr{c}\ord{r}$ equals $\ip{V_\mr{c}(R_0V_\mr{c})\ord{r-1}}_{\mr{L}}$.
  ...
  \begin{lem}
  \thmstatement{
    Restricting the bracketing expansion of $\Y\ord{n}$ to non-nested insertions and keeping only the linked contributions to each bracket $\ip{\cd}\mapsto\ip{\cd}_{\mr{L}}$ gives the linked component of the principal term, $((R_0V_\mr{c})^n\F)_{\mr{L}}$.
}
  \thmproof{
