...
This is actually an example of the Frantz-Mills factorization theorem, \Cref{thm:factorization}.}
...
\thmstatement{
A diagram with separate open parts equals the sum over all relative orderings of its parts in the corresponding combined diagram.
}
\thmproof{
  Apart from the resolvent lines, the separate and combined diagrams are identical and share a numerator, so we need only consider the denominators.
  Let $\mc{D}_1,\ld,\mc{D}_n$ be the denominator factors for one diagram with $n$ operators and let $\mc{D}_1',\ld,\mc{D}_{n'}'$ be the denominator factors for another diagram with $n'$ operators, ordered from bottom to top.
  We proceed by induction on $n+n'=m$.
  As a base case, consider $m=2$, of which the only non-trivial possibility is $n=n'=1$.
  There are two possible orderings for the combined diagram: either the operator of the first diagram is below the operator of the second diagram, or vice versa.
  In either case the upper resolvent crosses the lines of both diagrams, contributing a factor $\mc{D}_1+\mc{D}_1'$, but in the first case the lower resolvent crosses the lines of the first diagram and in the second case it crosses the lines of the second diagram.
...
$
  which confirms the proposistion for $m=2$.
  Now, suppose it holds for $m-1$ and consider $m$. 
  The different orderings for the combined graph can be grouped into two classes: those for which the first graph has the topmost operator, and those for which the second one does.
  All of the terms in either class have a common factor $\fr{1}{\mc{D}_n+\mc{D}_{n'}'}$ from the topmost resolvent.
  Factoring this out, the sum over the first class is equivalent to a sum over all orderings with the topmost operator of the first diagram removed.
  By our inductive assumption, this evaluates to 
  $
    \fr{
      1
    }{
  ...
  $.
  Similarly, after pulling out the common factor, the sum over the second class is equivalent to a sum over all orderings with the topmost operator of the second diagram removed, which evaluates to 
  $
    \fr{
      1
    }{
  ...
  Therefore, the sum over all orderings is
$
...
$
  which implies that the proposition holds for all $m$ and completes the proof for the case of two separate diagrams.
